...
public class OrderRepositoryImpl implements OrderRepositoryExt {
    ...
    public OrderRepositoryImpl(EntityManager em, ItemRepository itemRepo, AddressRepository aRepo, CartRepository cRepo, OrderItemRepository oiRepo, ItemService itemService) {
        this.em = em;
    ...
    public Optional<OrderEntity> insert(NewOrder m) {
        ...
        itemRepo.deleteCartItemJoinById(cart.getItems().stream().map(i ->i.getId()).collect(Collectors.toList()), cart.getId());
        OrderEntity entity = (OrderEntity) em.createNativeQuery("""
                SELECT o.* FROM ecomm.orders WHERE o.customer_id = ? AND o.order_date >= ?""", OrderEntity.class)
        ...
        oiRepo.saveAll(cart.getItems().stream().map( i -> new OrderItemEntity()
                .setOrderId(entity.getId()).setItemId(i.getId())).collect(Collectors.toList()));
